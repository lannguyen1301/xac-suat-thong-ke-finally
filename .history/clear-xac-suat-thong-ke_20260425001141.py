...
# --- BƯỚC 1: ĐỌC FILE KHÔNG DÙNG HEADER TỰ ĐỘNG ---
# Chúng ta đọc toàn bộ (header=None) để không bị mất dòng đầu tiên
df_raw = pd.read_excel(file_path, header=None, dtype=str)

# Tìm dòng thực sự chứa chữ "Ngay" hoặc "Ngày"
start_row = 0
for i, row in df_raw.iterrows():
    if "Ngay" in str(row[0]) or "Ngày" in str(row[0]):
        start_row = i
        break

# Lấy dữ liệu từ dòng sau dòng tiêu đề trở đi
