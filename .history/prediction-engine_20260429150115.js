...
// Dán cái này vào cuối file prediction-engine.js
window.renderPrediction = function () {
    const container = document.getElementById("prediction");
    if (!container) return;
    container.innerHTML = "";

    // Gọi hàm tính toán của bấy bề
    const finalData = getAdvancedPrediction(rawData);

    finalData.forEach((item) => {
        // Biến numbers này phải là mảng các số đã đủ chữ số (GDB 6 số...)
        const numbersHtml = item.numbers
            .map(
                (n) => `
            <div class="text-xl md:text-2xl font-black font-mono text-emerald-400 glow p-1">${n}</div>
        `,
            )
            .join("");
        ...
        container.innerHTML += `
            <div class="bg-slate-800 p-4 rounded-xl text-center border border-slate-700">
                <h3 class="text-xs text-gray-500 font-bold uppercase">${item.name}</h3>
                <div class="flex flex-wrap justify-center gap-2 mt-2">${numbersHtml}</div>
                <p class="text-[9px] text-gray-600 mt-2 italic">Độ tin cậy: ${item.score}%</p>
            </div>
        `;
    });
};
